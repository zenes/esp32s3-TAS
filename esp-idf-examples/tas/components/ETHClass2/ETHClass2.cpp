                         ...
                         int sck, int miso, int mosi, spi_host_device_t spi_host, uint8_t spi_freq_mhz)
{
    ...
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        log_e("spi_bus_initialize failed: %d", ret);
        return false;
    }
    ...
    phy_config.phy_addr = phy_addr;
    phy_config.reset_gpio_num = -1; // Disable internal reset to prevent conflict with already initialized chip

    // Reverted to Mode 0 as per user's advice (communication already verified at 1MHz)
    ...
    // W5500 allocation is now handled internally by the driver config in IDF 5.x
#if CONFIG_ETH_SPI_ETHERNET_W5500
    ...
    if (!initNetif((Network_Interface_ID)(ESP_NETIF_ID_ETH + _eth_index))) {
        return false;
    }
...
String ETHClass2::macAddress() const {
    uint8_t mac[6];
    char macStr[18] = {0};
    macAddress(mac);
...
bool ETHClass2::linkUp() const {
    ...
    return esp_eth_ioctl(_eth_handle, ETH_CMD_G_SPEED, &link_speed) == ESP_OK;
}
