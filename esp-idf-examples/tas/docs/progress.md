...
- **Dual LCD 유닛 지원을 위한 프로젝트 구조 개편**:
    - `platformio.ini`를 모듈화하여 Unit 1(ST7789+GT911)과 Unit 2(ILI9342+XPT2046) 환경을 독립적으로 선택 가능하도록 구조 개선.
    - `utilities.h` 및 `Setup217`에 유닛별 핀 정의(SPI MISO/MOSI/SCLK/CS 및 Touch CS/IRQ) 및 드라이버 옵션 추가.
- **터치 드라이버 통합 및 유연성 확보**:
    - `main.cpp`의 터치 읽기 콜백(`my_touchpad_read`)에서 GT911과 XPT2046 드라이버를 조건부 컴파일로 선택 가능하도록 구현.
    - Unit 2용 XPT2046 터치 컨트롤러 연동 및 좌표 매핑 로직 추가.
- **시스템 안정성 및 워치독 패닉(WDT) 문제 해결**:
    - LCD 하드웨어 감지 실패 시에도 시스템이 멈추지 않도록 `lv_timer_handler` 호출에 가드를 추가.
    - 메인 루프(`loop`)에 `delay(1)`을 추가하여 CPU 0의 IDLE 태스크가 실행될 시간을 확보함으로써 워치독 패닉 현상 해결.
- **LCD 하드웨어 감지 로직 강화**:
    - `detectLCD()` 함수에서 ST7789뿐만 아니라 ILI9341/9342 계열의 ID(0xD3)를 감지할 수 있도록 확장.
    - 사용자 요청에 따라 ID 감지 실패 시에도 초기화 프로세스를 강제로 진행(`Forcing initialization`)하도록 설정.
