...
*   **환경 설정 오류 수정**: `[lcd1-p8]` 환경 내 `LV_USE_PERF_MONITOR` 매크로 오타 수정.

## 2026-04-09
### evb-lcd1-p16(16비트 병렬) 컬러 왜곡 문제 해결 및 설정 최적화
*   **컬러 정합성 복구**: 노란색이 파란색/붉은색으로 출력되던 문제를 `invert = true` 및 `rgb_order = false`(RGB) 설정을 통해 완벽하게 해결.
*   **엔디안 설정 확정**: 16비트 병렬 인터페이스의 구조적 안정성을 위해 `LV_COLOR_16_SWAP=1` 설정을 유지하고, 이로 인한 비트 정렬 문제를 드라이버 수준에서 보정.
*   **설정 관리 체계 고도화**:
    *   `LGFX_Config.hpp` 내 하드웨어 종속적인 설정(Inversion, RGB Order)을 제거하고, `platformio.ini`의 빌드 플래그(`TFT_INVERSION_ON`, `TFT_RGB_ORDER`)를 이용해 환경별로 제어하도록 리팩토링.
    *   `lcd1-p16` 및 `lcd1-p8` 환경에 패널 특성 플래그를 추가하여 다른 하드웨어(`lcd3` 등)와의 호환성 확보.
*   **성능 및 안정성 최적화**: 
    *   진단 과정에서 낮췄던 병렬 버스 속도를 다시 **20MHz**로 복구하여 쾌적한 주사율 구현.
    *   `tas.ino` 내 디버깅용 배경색 코드를 제거하고 원래의 대시보드 UI 그라데이션 및 애니메이션 복구.
