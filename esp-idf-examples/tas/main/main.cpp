 ...
 * Ltd
 * @date      2024-02-03
 * @note      Converted from ESP-IDF tas example
...
void my_touchpad_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data) {
    ...
    #ifdef LCD_TYPE_ILI9341
    if (0) //if (tp.touched()) {
        TS_Point p; p.x=0; p.y=0; //TS_Point p = tp.getPoint();
    ...
    #elif defined(TOUCH_SDA) && defined(TOUCH_SCL)
    //tp.read();
    if (0) //if (tp.isTouched) {
        // Calibration mapping: 
        // X: 0 ~ 300 -> 0 ~ 319
        // Y: 85 ~ 315 -> 0 ~ 239
        int raw_x = 0; //int raw_x = tp.points[0].x;
        int raw_y = 0; //int raw_y = tp.points[0].y;
...
void initLVGLUI() {
  ...
  lv_label_set_text(title, "TAS Bridge");
