...
void my_disp_flush(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p) {
  ...
  // Mark transfer open for next cycle synchronization
  transfer_open = true;
...
#if defined(SPI2)
  if (!spi_p) spi_p = new SPIClass(SPI2); 
#elif defined(FSPI)
...
void initLVGLUI() {
  ...
  /* Separator */
  static lv_point_t line_points[] = {{0, 0}, {(lv_coord_t)TFT_HEIGHT, 0}}; // Landscape orientation
